...
-define(LB_CONF(Var, AccountId)
       ,kz_term:to_list(kz_json:get_binary_value([<<"mod_lb">>, Var], zz_util:reseller_vars(AccountId), <<"not_found">>))
       ).

-spec lb_login(kz_term:ne_binary()) -> any().
lb_login(AccountId) ->
...
-spec lb_logout(kz_term:ne_binary()) -> any().
lb_logout(AccountId) ->
    EncType = "application/x-www-form-urlencoded",
    Url = ?LB_CONF(<<"lb_url">>, AccountId),
    httpc:request(post, {Url, [], EncType, lists:flatten("devision=99")}, [], []).

-spec lb_soap_auth(atom(), kz_term:ne_binary()) -> any().
...
-spec maybe_unblock_agreement(kz_term:ne_binary(), any()) -> any().
...
-spec lb_soap_get_uid_by_number(kz_term:ne_binary(), any()) -> any().
